...
class EmsHelper
{
    ...
    /**
     * Refund EMS Online order.
     *
     * @param object $paymentMethod
     */
    public function orderRefund($paymentMethod)
    {
        $return_id = $paymentMethod->request->get['return_id'];
        $paymentMethod->load->model('sale/return');
        $paymentMethod->load->model('localisation/return_reason');
        $return_info = $paymentMethod->model_sale_return->getReturn($return_id);
        $return_reason = $paymentMethod->model_localisation_return_reason->getReturnReason($return_info["return_reason_id"]);

        if($paymentMethod->request->post['return_status_id'] == 3 ) {
            $orderId = $return_info['order_id'];
            $product = $return_info['model'];

            $paymentMethod->load->model('sale/order');

            $orderInfo = $paymentMethod->model_sale_order->getOrder($orderId);

            $orderProducts = $paymentMethod->model_sale_order->getOrderProducts($orderId);
            ...
            $order_history = $paymentMethod->model_sale_order->getOrderHistories($orderId, 1, 1);
            $emsOrderId =  substr($order_history[0]['comment'], strpos($order_history[0]['comment'], ":") + 2);


            $emsHelper = new EmsHelper($orderInfo['payment_code']);
            $ems = $emsHelper->getClientForAfterPay($paymentMethod->config);
